...
class DiscussionPostBloc extends Bloc<DiscussionPostEvent, DiscussionPostState> {
  ...
  DiscussionPostBloc({
    ...
    if (!(this.discussionBloc.state is DiscussionLoadedState) || this.discussionBloc.state.getDiscussion() == null) {
      discussionBlocSubscription = this.discussionBloc.listen(
        (DiscussionState state) {
          if (state is DiscussionLoadedState) {
            this.add(DiscussionPostsLoadedEvent(List<Post>.from(state.getDiscussion().posts)));
            discussionBlocSubscription.cancel();
          }
        }
      );
    } else if (this.discussionBloc.state.getDiscussion() != null) {
      this.add(DiscussionPostsLoadedEvent(List<Post>.from(this.discussionBloc.state.getDiscussion().posts)));
    } else {
      // Error state.
    }
  }
  ...
  Stream<DiscussionPostState> mapEventToState(
    ...
    if (event is DiscussionPostAddEvent && currentState is DiscussionPostLoadedState) {
      try {
        ...
        final addedPost = await repository.addPost(this.discussionID, event.postContent);
